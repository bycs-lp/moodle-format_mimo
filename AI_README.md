...
- **Multi-section mode**: Toggled per-course via `enablemultisection` option. When ON: course index activates, teachers can add/rename/reorder sections, each section displays its own wall with scoped filter bar and completion counts. **Overview landing page**: first visit (no `?section=` param) shows a card grid of all sections with activity counts and completion progress; clicking a card navigates to that wall; each wall has a "← Back to overview" link left of the section heading. When OFF (default): classic single-wall behavior with all activities in section 0.
...
5. **Learner view**
   ...
   - **Multi-section mode — overview**: First visit (no `?section=` param) shows a card grid of all sections. Each card displays section name, activity count, and completion progress bar. Clicking a card navigates to `?section=N`.
   - **Multi-section mode — single wall**: Shows one section's wall. A "← Back to overview" link appears left of the section heading, navigating to the overview (course URL without section param). The section name appears as an `<h2>` next to the back link.
...
- **Multi-section overview** (landing page): When no `?section=` param is present, `format.php` does NOT call `set_sectionnum()`. `content.php` detects `get_sectionid() === null` and builds lightweight section card data via `export_overview()`, using `overview.mustache`. This avoids rendering full walls for every section. The overview URL is the plain course URL without a section param.
- **Back to overview link**: In multi-section single-wall view, `content.php` exports `overviewurl`, `showoverviewlink`, and `currentsectionname`. The `content.mustache` template renders a "← Back to overview" link left of the section heading.
...
- `format.php` – entry point; branches on `is_multisection_enabled()`: multi-section skips `set_sectionnum()` when `$displaysection` is null (→ overview), sets it when a section is requested; single-section redirects non-0 and locks to section 0.
