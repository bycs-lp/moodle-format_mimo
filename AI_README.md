...
- **Course format base** (`lib.php`)
  ...
  - `can_delete_section()` — returns `true` for sections > 0 in multi-section mode; `false` in single-section mode.
    ...
    - `get_remembered_section()` — reads `format_mimo_lastsection_{courseid}` user preference (stores the **section ID**, stable across reordering), validates the section exists and is visible (section 0 always fails), clears stale preferences. Returns the section number or null.
  - Adds course options: `enablemultisection` (PARAM_BOOL, default `0`), `activityprofile` (PARAM_ALPHANUMEXT, selects which tagset to use; default = `profile_manager::get_default_profile_name()`, i.e. `base`), `enablefiltering`, `backgrounddesign` (PARAM_ALPHANUMEXT, default `'default'`; "default" falls back to the style's background, other values — `primary-school`, `darkmode`, `whiteboard`, `pinnwand`, `paper` — apply full-theme overrides to the board, cards, filter bar, navigation, and completion via a `.mimo-bgdesign-wrapper.mimo-bgdesign-{value}` wrapper and `.mimo-bgdesign-{value}` on the `<ul>` board), `distractionfree`.
...
- **Tag domain model** (`db/install.xml` + `classes/tag_manager.php`)
  - Table: `*_tags` (name, bgcolor hex, imgplacement center|lower, cardimage, filterimage, activitytype1-3, sortorder, **scope** CHAR(10) DEFAULT 'global' — values: 'global' or 'imported'). **Equal-tagset semantics:** these rows are invisible *anchors* holding creation-time values; the per-set appearance lives entirely in `*_profile_tags`. Anchors are kept for cmtag FKs and restore fingerprinting and are never edited via the UI after creation.
  ...
  - File areas (`FILEAREA_CARDIMAGE = 'tagcard'`, `FILEAREA_FILTERIMAGE = 'tagfilter'`) in system context; served via `format_mimo_pluginfile()`. Accepted types: `.svg`, `.png`. **Strict per-set images:** `get_cardimage_url($tag, $profilename)` / `get_filterimage_url()` resolve ONLY the profile file area when a profile name is given — no fallback to the anchor areas. A set without its own image shows none.
  - Key methods: `create_tag($name, ..., $scope = 'global', $createdinprofileid = null)` (when a profile id is given, materializes one `enabled=1` row in that set — all other sets stay disabled), `get_all_tags()`, `get_tags_for_course($courseid)` (returns profile-filtered tags including imported tags bound to the course), `assign_tag_to_cm($cmid, $tagid)`, `get_cm_tag($cmid)`, `is_tag_disabled_everywhere($tagid)` (no `enabled=1` row exists; missing rows count as disabled).
  - **Install-time activity type sanitization**: `sanitize_default_activitytypes($t1, $t2, $t3)` checks each suggested activity type against the installed modules; missing modules are replaced via the `ACTIVITYTYPE_FALLBACKS` map (`hvp` → `h5pactivity`, `kanban` → `forum`) when the fallback is installed, otherwise dropped (e.g. `board` has no core alternative). Results are de-duplicated and shifted up so empty slots trail. Applied in `create_tag_from_default()` (anchor seeding) and in `profile_manager::apply_default_profile_tag_overrides()` (override trio merged with the base definition before sanitizing). Install-time only — runtime values are not filtered.
  - **Imported tag methods**: `bind_tag_to_course($tagid, $courseid)`, `unbind_tag_from_course($tagid, $courseid)`, `unbind_all_tags_from_course($courseid)`, `promote_tag_to_global($tagid)` (sets scope='global', deletes all bindings), `get_imported_tags_for_course($courseid)`, `cleanup_orphaned_imported_tags()` (deletes imported tags with zero bindings and zero cmtags), `find_tag_by_fingerprint($data, $excludeids)` (NULL-safe composite match on name+bgcolor+activitytype1-3 against ANCHOR values).
  - `get_tags_for_course()` reads the course's `activityprofile` option (falls back to `profile_manager::get_default_profile_name()` for empty/stale values), fetches imported tags bound to the course via `get_imported_tags_for_course()`, merges them with global tags, then resolves through `profile_manager::resolve_tags_for_profile()`.
  - `delete_tag()` cascades to profile_tags, cmtags, **and course_tags bindings**. Only reachable when `is_tag_disabled_everywhere()` (enforced server-side in `tag_management.php`).
...
- **Profile system** (`classes/profile_manager.php`)
  ...
  - Table: `*_profile_tags` (tagid+profileid unique; per-set values for name, bgcolor, activitytype1-3, imgplacement, imgsize; `enabled` flag). **Materialization invariant:** rows are always fully filled (no NULL = inherit). A missing row resolves as **disabled**. Two representations of disabled coexist: explicit `enabled=0` row (preserves per-set values) and no row at all (never configured).
  - Key methods: `create_profile($name, $displayname, $scope = 'global')` (creates NO profile_tags rows — new tagsets start empty/all-disabled), `get_profile_by_name($name)`, `get_or_create_profile_tag($tagid, $profileid)` (new rows are materialized from anchor values with `enabled=0`), `materialize_profile_tag($tagid, $profileid, $values, $enabled)` (upsert: $values → existing non-NULL → anchor; the single write path for per-set data), `materialize_all_profile_tags($enablemissing)` (upgrade/install helper), `copy_base_images_to_profile_tags()` (one-time anchor→profile image copy), `update_profile_tag($id, $data)`, `resolve_tags_for_profile($tags, $profileid, $onlyenabled)`, `resolve_tag_for_profile($tag, $profileid)` (missing row → `enabled=0`, anchor values returned for greyed display), `get_default_profile_name()` (first global profile by sortorder — used for ALL default/fallback sites instead of hardcoded names).
  ...
  - Profiles (tagsets) are **equal peers**: the former "base tag fields" master view is gone. The `base` profile (seeded first, sortorder 0) is a regular set like any other. Editing a tag in one set never affects another set.
  - **Profile dropdown filtering**: `lib.php` course_format_options uses `get_global_profiles()` for the dropdown, then adds the current course's imported profile if already assigned. Other courses don't see imported profiles in their dropdowns. New courses default to `get_default_profile_name()` (= `base`).
  - Default profiles (created on install): `base` (sortorder 0, plain anchor values), `base_symbols` (1, base values with pictogram artwork), `primary_horst` (2). `initialize_default_profiles()` shifts existing sortorders +1 when inserting `base`, applies the per-profile overrides, then calls `materialize_all_profile_tags(true)`.
  - **Default profile overrides**: Uses a generic data structure (profile name → tag index → override fields). Supports any field: `name`, `bgcolor`, `activitytype1-3`, `enabled`, `imgplacement`, `imgsize`, `cardimage`, `filterimage`. Image fields trigger a file copy from `pix/tags/` into the profile file area via `copy_default_profile_image()` + DB field update via `apply_default_profile_images()`. Anything *not* overridden is inherited from the anchor definition in `tag_manager::get_default_tag_definitions()` (e.g. `imgplacement = lower` is set there once and applies to `base`, `base_symbols` and `primary_horst`; horst additionally overrides `imgsize = bigger`).
- **Section image system** (`classes/section_image_manager.php`)
  ...
  - Key methods: `get_image_url($courseid, $sectionid)`, `save_image($courseid, $sectionid, $draftitemid)`, `delete_image($courseid, $sectionid)`, `has_image($courseid, $sectionid)`, `delete_all_for_course($courseid)`, `normalize_fit($fit)`.
  - When a section has an uploaded image, it replaces the miniwall tiles on the overview card.
  - **Image fit** (`sectionimagefit` section format option, `PARAM_ALPHANUMEXT`, default `'contain'`): `contain` (show full image) or `cover-top`/`cover-center`/`cover-bottom` (fill area via `object-fit: cover` + `object-position`, cropping the opposite edge). `section_image_manager::normalize_fit()` maps the legacy single `'cover'` value (pre-variant data) to `'cover-center'` and falls back to `DEFAULT_FIT` for anything unrecognised; both `content.php` (overview rendering) and `section_image_form.php` (edit form default) call it before use.
...
- **Event observers** (`classes/observer.php` + `db/events.php`)
  ...
  - `course_section_deleted`: Delete section overview card image for the deleted section + eagerly delete remembered-section preferences pointing at the deleted section (exact match on stored section ID).
  - `course_deleted`: Delete all orphaned cmtag records + delete all section images for the course + **delete course_tags bindings + cleanup orphaned imported tags + cleanup orphaned imported profiles** + delete done flags for course + delete remembered-section (`format_mimo_lastsection_{courseid}`) preferences for all users + clear caches.
- **Done system** (`classes/done_manager.php` + `classes/courseformat/stateactions.php`)
  ...
  - **DI pattern (ALL managers)**: every manager class (`tag_manager`, `profile_manager`, `done_manager`, `section_image_manager`, `completion_defaults_manager`, `description_tag_manager`, `activity_description_manager`, `completion_helper`) is an instance class resolved via `\core\di::get(manager::class)` with constructor-injected `\core\clock` where needed (section_image_manager and completion_helper have no constructor). **Database access uses the conventional `global $DB` inside methods — never constructor-inject `\moodle_database`** (a DI-cached manager would hold a stale DB reference when the global connection is swapped, e.g. in tests or install/restore). Class constants (FILEAREA_*, etc.) remain static. Cross-manager calls use lazy `\core\di::get()` inside methods (never constructor injection — tag_manager↔profile_manager would be circular). PHPUnit/Behat reset the DI container between tests, so the old static-cache reset hooks (`tag_manager::reset_caches()`, `completion_helper::reset_caches()`, done_manager's `reset_cache()`) were deleted. Tests hold instances in properties initialized in `setUp()`; static contexts (data providers, Behat generators) use inline `di::get()`.
...
- **Sticky wall** (`format_mimo_lastsection_{courseid}` user preference): Visiting a wall stores the **section ID** (not the number — IDs are stable across reordering/deletion). Returning to the course without `?section=` auto-redirects to the last wall. `?overview=1` clears the preference and shows overview. `extend_course_navigation()` also stores the preference when viewing an activity page (deep link and course index activity clicks). `get_remembered_section()` resolves the ID via `get_section_info_by_id()`, validates visibility, and returns the current section number. The `course_section_deleted` observer eagerly deletes preferences pointing at the deleted section; the `course_deleted` observer cleans up the preference for all users on course deletion.
...
- Never bypass the profile system — `get_tags_for_course()` filters tags through the course's activity profile. **Equal-tagset semantics:** a tag is only active in a set with an explicit `profile_tags` row with `enabled=1`; a missing row means disabled. Never write NULL override fields — always go through `profile_manager::materialize_profile_tag()`.
- Every course should have a valid `activityprofile` option — defaults to `profile_manager::get_default_profile_name()` (first global profile by sortorder, i.e. `base`). Never hardcode profile names in defaults/fallbacks.
- **Per-set enable/disable**: the eye toggle on `tag_management.php` (`action=toggletag`) is the single control for the `enabled` flag (the edit modal has no enabled checkbox). Deleting a tag is only allowed once it is disabled in every set (`is_tag_disabled_everywhere()`, enforced in UI and server-side).
- **Strict per-set images**: profile image resolution has NO fallback to the anchor (`tagcard`/`tagfilter`) areas. The anchor areas only keep creation-time uploads for fingerprint/back-compat.
...
- **Manager DI pattern**: all `format_mimo` managers are instance classes obtained via `\core\di::get(manager::class)`. Never add new static manager methods; never inject one manager into another's constructor (circularity) — use lazy `\core\di::get()` inside the method instead. Never constructor-inject `\moodle_database` — use `global $DB` inside methods.
...
- Backup/restore reuses by fingerprint match on same instance (fingerprints compare ANCHOR values, frozen at creation). Don't create duplicate-prevention logic elsewhere. Restore writes fully materialized imported-profile rows and copies resolved images into the imported profile's own file areas.
...
- `classes/profile_manager.php` – profile CRUD, per-set tag values (name, bgcolor, activity types, enabled, images — always fully materialized), tag resolution (missing row = disabled), **imported profile creation/promotion/cleanup**, **default profile seeding incl. `base`**. Key methods: `resolve_tags_for_profile()`, `resolve_tag_for_profile()`, `materialize_profile_tag()`, `materialize_all_profile_tags()`, `copy_base_images_to_profile_tags()`, `get_or_create_profile_tag()`, `get_default_profile_name()`, `create_imported_profile()`, `promote_profile_to_global()`, `cleanup_orphaned_imported_profiles()`, `get_global_profiles()`.
...
- `tag_management.php` – admin UI controller for tagsets and tags: equal tagset buttons (base first, default view = default profile), create tag in any set, per-set eye toggle (`action=toggletag`), delete gated by `is_tag_disabled_everywhere()` (server-enforced), **promote actions for imported tags/profiles**.
...
- `backup/moodle2/restore_format_mimo_plugin.class.php` – restore handler (**four-tier fingerprint/name/positional/create matching, imported profile creation with fully materialized rows + per-set image copies**, ID mapping, format option remapping).
...
- **TODO — `sr` → `returnoptions[pagesectionid]` dual-path (MDL-86284, Moodle 5.3)**: Moodle 5.3+/main deprecates the `sr` URL param/option (modedit.php emits a `debugging()` on it, which is fatal in Behat). The plugin currently supports 5.2 AND 5.3+ from one branch via capability detection: the `activitychooserbutton` exporter checks `method_exists($format, 'get_return_options')` and exports `pagesectionid` (5.3+) alongside the legacy `sectionreturn`/`sectionreturnid`; `tagchooserbutton.js` prefers `returnoptions[pagesectionid]` and falls back to `sr`; `lib.php get_view_url()` accepts both a `pagesectionid` (section ID) and an `sr` (section number) option. **Revisit and remove the legacy `sr` path** when (a) 5.2 support is dropped or MDL-86284 lands in a 5.2.x point release, or (b) the plugin is split into per-branch releases (502 / 503+) — then each branch keeps only its own path. Core plans full `sr` removal in Moodle 7.0 (MDL-88498).
