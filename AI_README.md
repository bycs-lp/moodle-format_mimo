...
- **Manager DI pattern**: all `format_mimo` managers are instance classes obtained via `\core\di::get(manager::class)`. Never add new static manager methods; never inject one manager into another's constructor (circularity) — use lazy `\core\di::get()` inside the method instead.
