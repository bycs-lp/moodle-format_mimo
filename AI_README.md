...
- **Profile system** (`classes/profile_manager.php`)
  ...
  - Default profiles (created on install): `base` (sortorder 0, plain anchor values), `primaryschool` (1), `secondaryschool` (2), `foreignlanguage` (3). `initialize_default_profiles()` shifts existing sortorders +1 when inserting `base`, applies the per-profile overrides, then calls `materialize_all_profile_tags(true)`.
