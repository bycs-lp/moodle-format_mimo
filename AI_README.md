...
- **Tag domain model** (`db/install.xml` + `classes/tag_manager.php`)
  ...
  - **Install-time activity type sanitization**: `sanitize_default_activitytypes($t1, $t2, $t3)` checks each suggested activity type against the installed modules; missing modules are replaced via the `ACTIVITYTYPE_FALLBACKS` map (`hvp` → `h5pactivity`) when the fallback is installed, otherwise dropped (e.g. `kanban`, `board` have no core alternative). Results are de-duplicated and shifted up so empty slots trail. Applied in `create_tag_from_default()` (anchor seeding) and in `profile_manager::apply_default_profile_tag_overrides()` (override trio merged with the base definition before sanitizing). Install-time only — runtime values are not filtered.
...
- **Profile system** (`classes/profile_manager.php`)
  ...
  - Default profiles (created on install): `base` (sortorder 0, plain anchor values), `base_symbols` (1, base values with pictogram artwork), `primary_horst` (2), `foreignlanguage_english` (3). `initialize_default_profiles()` shifts existing sortorders +1 when inserting `base`, applies the per-profile overrides, then calls `materialize_all_profile_tags(true)`.
