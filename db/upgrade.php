...
function xmldb_format_mimo_upgrade($oldversion) {
    ...
    if ($oldversion < 2025112302) {
        upgrade_plugin_savepoint(true, 2025112302, 'format', 'mimo');
    }

    if ($oldversion < 2025112500) {
        $table = new xmldb_table('format_mimo_tags');
        ...
        if (!empty($palette)) {
            ...
            foreach ($tags as $tag) {
                ...
                $DB->set_field('format_mimo_tags', 'bgcolor', $color, ['id' => $tag->id]);
                $index++;
            }
        }

        upgrade_plugin_savepoint(true, 2025112500, 'format', 'mimo');
    }

    if ($oldversion < 2025112501) {
        // Define table format_mimo_actdesc to be created.
        $table = new xmldb_table('format_mimo_actdesc');

        // Adding fields to table format_mimo_actdesc.
        ...
        // Adding keys to table format_mimo_actdesc.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Adding indexes to table format_mimo_actdesc.
        $table->add_index('activitytype_unique', XMLDB_INDEX_UNIQUE, ['activitytype']);

        // Conditionally launch create table for format_mimo_actdesc.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        upgrade_plugin_savepoint(true, 2025112501, 'format', 'mimo');
    }

    if ($oldversion < 2025112800) {
        // Add activitytype3 field to format_mimo_tags table.
        $table = new xmldb_table('format_mimo_tags');
        ...
        upgrade_plugin_savepoint(true, 2025112800, 'format', 'mimo');
    }

    if ($oldversion < 2025112801) {
        // Drop unused description fields from tagsets and tags tables.
        $table = new xmldb_table('format_mimo_tagsets');
        ...
        upgrade_plugin_savepoint(true, 2025112801, 'format', 'mimo');
    }

    if ($oldversion < 2025112802) {
        // Create description tags table.
        $table = new xmldb_table('format_mimo_desc_tags');

        // Adding fields to table format_mimo_desc_tags.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        // Adding keys to table format_mimo_desc_tags.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Conditionally launch create table for format_mimo_desc_tags.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
        ...
        $key = new xmldb_key('desctagid', XMLDB_KEY_FOREIGN, ['desctagid'], 'format_mimo_desc_tags', ['id']);
        $dbman->add_key($table, $key);

        upgrade_plugin_savepoint(true, 2025112802, 'format', 'mimo');
    }

    if ($oldversion < 2025112803) {
        // Rename tagid to desctagid in format_mimo_actdesc table.
        $table = new xmldb_table('format_mimo_actdesc');
        $field = new xmldb_field('tagid', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'description');

        if ($dbman->field_exists($table, $field)) {
            // Drop old foreign key and index first.
            $key = new xmldb_key('tagid', XMLDB_KEY_FOREIGN, ['tagid'], 'format_mimo_desc_tags', ['id']);
        ...
        upgrade_plugin_savepoint(true, 2025112803, 'format', 'mimo');
    }

    if ($oldversion < 2025120300) {
        // Add imgplacement field to format_mimo_tags table.
        $table = new xmldb_table('format_mimo_tags');
    ...
    if ($oldversion < 2026010700) {
        ...
        $key = new xmldb_key('tagsetid', XMLDB_KEY_FOREIGN, ['tagsetid'], 'format_mimo_tagsets', ['id']);
        $dbman->drop_key($table, $key);
        ...
        // Step 7: Clear caches.
        $cache = cache::make('format_mimo', 'tagconfigurations');
        $cache->purge();
        $cache = cache::make('format_mimo', 'activitytagmappings');
        $cache->purge();

        upgrade_plugin_savepoint(true, 2026010700, 'format', 'mimo');
    }

    if ($oldversion < 2026010800) {
        // Create designs table.
        $table = new xmldb_table('format_mimo_designs');
        ...
        $table = new xmldb_table('format_mimo_tag_images');
        ...
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('tagid', XMLDB_KEY_FOREIGN, ['tagid'], 'format_mimo_tags', ['id']);
        $table->add_key('designid', XMLDB_KEY_FOREIGN, ['designid'], 'format_mimo_designs', ['id']);
        ...
        foreach ($designs as $design) {
            if (!$DB->record_exists('format_mimo_designs', ['name' => $design['name']])) {
                $record = (object) $design;
                $record->timecreated = $now;
                $record->timemodified = $now;
                $DB->insert_record('format_mimo_designs', $record);
            }
        }

        // Migrate existing tag images to the first design (classic).
        $classicdesign = $DB->get_record('format_mimo_designs', ['name' => 'classic']);
        if ($classicdesign) {
            ...
            foreach ($tags as $tag) {
                ...
                if (!$DB->record_exists('format_mimo_tag_images', [
                    ...
                    $record->timecreated = $now;
                    $record->timemodified = $now;
                    $DB->insert_record('format_mimo_tag_images', $record);
                }
            }
        }

        upgrade_plugin_savepoint(true, 2026010800, 'format', 'mimo');
    }
    ...
    if ($oldversion < 2026020900) {
        ...
        $tagtable = new xmldb_table('format_mimo_tags');
        ...
        if ($defaulttagsetid) {
            $DB->set_field('format_mimo_tags', 'tagsetid', $defaulttagsetid);
        }
        ...
        // Step 8: Set tagsetid on all existing mimo courses.
        // Without this, editing a course post-upgrade fails validation (tagsetid = 0).
        if ($defaulttagsetid) {
            $courses = $DB->get_records('course', ['format' => 'mimo']);
            foreach ($courses as $course) {
                $existing = $DB->get_record('course_format_options', [
                    'courseid' => $course->id,
                    'format' => 'mimo',
        ...
        upgrade_plugin_savepoint(true, 2026020900, 'format', 'mimo');
    }

    if ($oldversion < 2026021000) {
        // Rename "design" to "style" across all tables, columns, file areas, and format options.

        // Step 1: Rename table format_mimo_designs -> format_mimo_styles.
        $table = new xmldb_table('format_mimo_designs');
        if ($dbman->table_exists($table)) {
            $dbman->rename_table($table, 'format_mimo_styles');
        }
        ...
        $key = new xmldb_key('designid', XMLDB_KEY_FOREIGN, ['designid'], 'format_mimo_styles', ['id']);
        $dbman->drop_key($table, $key);
        ...
        $key = new xmldb_key('styleid', XMLDB_KEY_FOREIGN, ['styleid'], 'format_mimo_styles', ['id']);
        $dbman->add_key($table, $key);
        ...
        $DB->execute(
            "UPDATE {course_format_options}
              ...
              WHERE format = 'mimo' AND name = 'designvariant'"
        );
        ...
        $DB->execute(
            "UPDATE {files}
              ...
              WHERE component = 'format_mimo' AND filearea = 'designtagcard'"
        );
        $DB->execute(
            "UPDATE {files}
                SET filearea = 'styletagfilter'
              WHERE component = 'format_mimo' AND filearea = 'designtagfilter'"
        );
    ...
    if ($oldversion < 2026022600) {
        ...
        if ($dbman->table_exists($table)) {
            $dbman->rename_table($table, 'format_mimo_profiles');
        }
        ...
        if ($dbman->table_exists($table)) {
            ...
            $key = new xmldb_key('styleid', XMLDB_KEY_FOREIGN, ['styleid'], 'format_mimo_profiles', ['id']);
            $dbman->drop_key($table, $key);
            ...
            $key = new xmldb_key('profileid', XMLDB_KEY_FOREIGN, ['profileid'], 'format_mimo_profiles', ['id']);
            $dbman->add_key($table, $key);

            // Rename the table.
            $dbman->rename_table($table, 'format_mimo_profile_tags');
        }
        ...
        $DB->execute(
            "UPDATE {course_format_options}
              ...
              WHERE format = 'mimo' AND name = 'stylevariant'"
        );
        ...
        $DB->execute(
            "UPDATE {files}
              ...
              WHERE component = 'format_mimo' AND filearea = 'styletagcard'"
        );
        $DB->execute(
            "UPDATE {files}
    ...
    if ($oldversion < 2026022601) {
        $DB->delete_records_select(
            ...
            ['format' => 'mimo', 'name' => 'selectedtags']
        );

        upgrade_plugin_savepoint(true, 2026022601, 'format', 'mimo');
    }
    ...
    if ($oldversion < 2026022800) {
        ...
        $table = new xmldb_table('format_mimo_profile_tags');
        ...
        upgrade_plugin_savepoint(true, 2026022800, 'format', 'mimo');
    }
    ...
    if ($oldversion < 2026022801) {
        ...
        $table = new xmldb_table('format_mimo_compdefs');
        if (!$dbman->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        upgrade_plugin_savepoint(true, 2026022801, 'format', 'mimo');
    }
    ...
    if ($oldversion < 2026030500) {
        ...
        upgrade_plugin_savepoint(true, 2026030500, 'format', 'mimo');
    }
            ...
            $DB->execute(
                "UPDATE {course_format_options}
                  ...
                  WHERE format = ?
                    AND name = ?
                    AND {$valuefield} = ?",
                [$newval, 'mimo', 'backgrounddesign', $oldval]
            );
        }

        upgrade_plugin_savepoint(true, 2026030501, 'format', 'mimo');
    }
        ...
        $DB->execute(
            "UPDATE {course_format_options}
              ...
              WHERE format = ?
                AND name = ?
                AND {$valuefield} = ?",
            ['primary-school', 'mimo', 'backgrounddesign', 'default']
        );
    ...
    if ($oldversion < 2026031401) {
        ...
        $table = new xmldb_table('format_mimo_profiles');
        ...
        $table = new xmldb_table('format_mimo_course_tags');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
